...
Dieses Programm funktioniert ausschließlich in Verbindung mit einer vorhandenen und korrekt eingerichteten Alarmverarbeitung.

Die empfohlene und getestete Umsetzung ist der in der Datei
"Node-Red-Aufbau.txt" beschriebene Aufbau.

Wichtig:

Der Client ist auf die dort definierte Schnittstelle ausgelegt
Der Server muss einen kompatiblen HTTP-Endpunkt bereitstellen
Die Datenstruktur (JSON) muss dem beschriebenen Format entsprechen

Andere Lösungen, insbesondere individuell entwickelte Systeme ("Eigenbau"),
können grundsätzlich verwendet werden, jedoch gilt:

Es wird nicht gewährleistet, dass diese Systeme vollständig kompatibel sind
oder reibungslos funktionieren.

Für abweichende Implementierungen ist der Anwender selbst verantwortlich.

WICHTIGER HINWEIS ZUR KONFIGURATION

Die in der Datei "config.json" eingetragene Serveradresse ist lediglich
eine Beispieladresse und muss zwingend angepasst werden.

Ohne Anpassung der Serveradresse ist keine Verbindung zum Alarmserver möglich.

Folgender Eintrag muss überprüft und geändert werden:

"server_url": "http://IP_DES_SERVERS:1880/alarm-status"

Hier ist die tatsächliche IP-Adresse bzw. der Hostname des eigenen
Alarmservers einzutragen.
